...
export class CatalogMainPage extends BasePage {
  //LOCATORS
  private locatorL1PcAndNetworsLocator =
    "//span[contains(@class,'catalog-navigation-classifier__item-title-wrapper') and contains(text(),'Компьютеры и сети')]";
  private locatorL1Electronics =
    "//span[contains(@class,'catalog-navigation-classifier__item-title-wrapper') and contains(text(),'Электроника')]";
  private locatorL2TVandVideo =
    "//div[text()[normalize-space()='Телевидение и видео']]";
  private locatorL3Notebooks =
    "//span[contains(@class,'catalog-navigation-list__dropdown-title') and contains(text(),'Ноутбуки')]";
  private locatorL3TVs =
    "//span[contains(@class,'catalog-navigation-list__dropdown-title') and contains(text(),'Телевизоры')]";
  ...
  async openNotebookCatalog() {
    ...
    if (await this.l3Notebooks.isVisible()) {
      ...
      await this.openNotebookCatalog();
    }
