...
export class realtPage extends BasePage{
    ...
    private locatorCounterSearchResults = "//div[@class='classifieds-bar__group']//div[1]";
    private locatorFilterFlats = "//span[text()='Квартира']";
    private locatorSearchResultType = "//span[@class='classified__caption-item classified__caption-item_type']";
    ...
    private locatorFilterMetroNearMetro = "//li[text()[normalize-space()='Возле метро']]";
    private locatorSortingDropdown = "//div[contains(@class,'dropdown dropdown_right')]";
    private locatorSortingExpensiveFirst = "//li[text()='Сначала дорогие']";
    private locatorResultAdress = "//span[@data-bind='text: apartment.location.user_address']";
    ...
    async setSortingExpensiveFirst(){
        ...
        await expect(async()=> {
            expect(adressOfFirstResult).not.toEqual(await this.resultAdress.first().innerText())
        }).toPass()
    }
    ...
    async checkMaxPriceOfResults(filterMaxPrice:number){
        (await this.filterResultUSDprice.all()).forEach(async (resultPrice) => {
            expect(await getInnerNumber(resultPrice)).toBeLessThan(filterMaxPrice+1);
        })
    }
