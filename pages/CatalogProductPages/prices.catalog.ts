...
export class PricesPage extends BaseCatalog {
  //LOCATORS
  private locatorSectionOfferList =
    "//div[@class='offers-filter__part offers-filter__part_2']";
  private locatorDropdownSortingOffersList =
    "//select[@class='input-style__real']";
  ...
  private locatorGreenButtonAddedinCart =
    "//div[@class = 'offers-list__control offers-list__control_default helpers_hide_tablet']//a[@class='button-style button-style_base-alter offers-list__button offers-list__button_cart button-style_another']";
  private locatorSectionRecommendedSidebar =
    "//div[@class = 'product-recommended__sidebar-overflow']";
  private locatorButtonCloseRecommendedSidebar = `//div[@class='product-recommended__sidebar-close']`;
  private locatorPriceInOfferTile =
    "//div[@class='offers-list__description offers-list__description_alter-other offers-list__description_huge-alter offers-list__description_font-weight_bold offers-list__description_ellipsis offers-list__description_nodecor']";
  ...
  async sortOffers_ascending() {
    this.dropdownSortingOffersList.selectOption(
      this.optionsDropdownSortingOffersList.ascending,
    );
  ...
  async getPriceInOfferTile(elementOfferTile: Locator) {
    ...
    let price = await getInnerNumber(
      elementOfferTile.locator(this.priceInOfferTile),
    );
    return price;
  }
}
