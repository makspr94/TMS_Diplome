...
export class CustomerSupportPage extends BasePage {
  ...
  private locatorFieldDescription =
    "//textarea[@name='supportform[description]']";
  private locatorFieldCaptcha =
    "//input[@name='supportform[captcha_keystring]']";
  ...
  async checkDrowdownTracker() {
    ...
    await expect(this.dropdownTracker).toContainText(
      this.optionsDropdownTracker.bug,
    );
    await this.dropdownTracker.selectOption(
      this.optionsDropdownTracker.feature,
    );
    ...
    await this.dropdownTracker.selectOption(
      this.optionsDropdownTracker.newfunction,
    );
  ...
  async checkDrowdownCategory() {
    await this.dropdownCategory.selectOption(
      this.optionsDropdownCategory.baralog,
    );
    await expect(this.dropdownCategory).toContainText(
      this.optionsDropdownCategory.baralog,
    );
    await expect(this.dropdownCategory).toHaveValue(/baralog/);

    await this.dropdownCategory.selectOption(
      this.optionsDropdownCategory.catalog,
    );
    await expect(this.dropdownCategory).toContainText(
      this.optionsDropdownCategory.catalog,
    );
    ...
    await this.dropdownCategory.selectOption(
      this.optionsDropdownCategory.forum,
    );
    await expect(this.dropdownCategory).toContainText(
      this.optionsDropdownCategory.forum,
    );
