...
class Posts {
    ...
    async getNonExistentUser(){
        const allUsers: number[] = this.getAllUniqueValuesByKey('userId', await this.getAll());
    ...
    async getRandomPostId() {
        let allPostsIdArray: number[] = this.getAllUniqueValuesByKey('id', await this.getAll());
        let randomPostId: number = Math.floor(Math.random()*allPostsIdArray.length);
        return randomPostId;
    };
