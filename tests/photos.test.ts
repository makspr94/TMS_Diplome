...
import {
  photoForCreationAlbum5,
  photoForCreationWithoutAlbumId,
} from "../data/photos/photosTestData";
...
describe("photos tests", () => {
  ...
  test("user uploads new photo", async () => {
    const existingPhotos = photos.getAllUniqueValuesByKey(
      "id",
      await photos.getAll(),
    );
  ...
  test("NEGATIVE-404: user cannot upload new photo without albumId", async () => {
    const response = await photos.createNewPhoto(
      photoForCreationWithoutAlbumId,
    );
  ...
  test("NEGATIVE-404: user cannot create photo with non-existing album", async () => {
    ...
    const response = await photos.createNewPhoto(
      createPhotoForNonExistentAlbum,
    );
    expect(response.status).toEqual(404);
