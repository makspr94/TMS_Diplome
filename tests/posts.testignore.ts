...
describe( "posts tests", () => {
   ...
   // - Пользователь может получить пост по его Id
    ...
    test ('get all posts by userID', async () => {
        ...
        expect(+(posts.getAllUniqueValuesByKey('userId', response))).toEqual(randomUsedId); 
    });
    ...
    test ('get all comments by Post Id', async () => {
        ...
        const postIdOfRecievedComments: number = +(posts.getAllUniqueValuesByKey('postId', response));
        expect(response.status).toEqual(200);
        expect(postIdOfRecievedComments).toEqual(randomPostId);
        expect(response.data.length).toEqual(await posts.countCommentsWithSpecificPostId(randomPostId));
    });
...
/*
  ...
  3. повторяется код в таких методах как getAllPostsId и getAllUsersId. Можно вывести в какой-то отдельный метод
  4. error toJSON - is it really needed?
  */
